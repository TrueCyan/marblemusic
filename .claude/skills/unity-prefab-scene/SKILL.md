...
prefab-tool delete-object Scene.unity --id 12345 --force    # 확인 없이 삭제
```
...
prefab-tool delete-component Scene.unity --id 67890 --force  # 확인 없이 삭제
```
...
## Unity UI 설계 베스트 프랙티스

### 핵심 원칙

#### 1. 한 오브젝트 = 하나의 레이아웃
```
❌ 금지: 같은 오브젝트에 VLG + HLG 동시 사용
✅ 올바름: 오브젝트당 하나의 LayoutGroup만 사용
```

VLG(VerticalLayoutGroup)와 HLG(HorizontalLayoutGroup)를 같은 오브젝트에 추가하면 레이아웃 충돌이 발생합니다.

#### 2. Row 단위 그룹화

UI 요소를 기능별로 Row 단위로 묶습니다:
...
| 접미사 | 용도 | 예시 |
|--------|------|------|
| `Panel` | 컨테이너 | BeatDivisionPanel, BPMPanel |
| `Row` | 가로 그룹 | ControlRow, SnapRow |
| `Title` | 라벨/제목 | DivisionTitle |
| `Btn` | 버튼 | MinusBtn, PlusBtn |
| `Text` | 텍스트 표시 | ValueText |
...
불필요한 중첩 피하기:
```
❌ Panel > Container > Wrapper > Row > Element
✅ Panel > Row > Element
```

#### 5. ContentSizeFitter 주의사항

```
❌ 부모와 자식 모두에 CSF 사용 → 순환 의존성 발생
✅ 최상위 컨테이너에만 CSF 사용
```

**순환 의존성 예시 (문제):**
```
RightTopPanel (VLG + CSF)
└── BeatDivisionPanel (VLG + CSF)  ← 충돌!
```

**해결:**
```
RightTopPanel (VLG + CSF)
└── BeatDivisionPanel (VLG만)  ← CSF 제거
```

#### 6. LayoutGroup 설정 가이드

```yaml
# 부모 VLG (자식 높이 제어)
m_ChildControlWidth: 1
m_ChildControlHeight: 1  # 자식 높이 제어
m_ChildForceExpandWidth: 1
m_ChildForceExpandHeight: 0

# 자식 HLG (Row)
m_ChildControlWidth: 0  # 자식 너비는 자체 설정
m_ChildControlHeight: 0
m_ChildForceExpandWidth: 0
m_ChildForceExpandHeight: 0
```

### 실제 예시: BeatDivisionPanel

```
BeatDivisionPanel (VLG)
├── DivisionTitle (TextMeshProUGUI) - "Division"
├── DivisionRow (HLG)
│   ├── MinusBtn (Button)
│   ├── ValueText (TextMeshProUGUI) - "4"
│   └── PlusBtn (Button)
└── SnapRow
    └── SnapToggleBtn (Button)
```

**컴포넌트 구성:**
- BeatDivisionPanel: RectTransform, Image, VLG
- DivisionRow: RectTransform, HLG
- 개별 버튼/텍스트: RectTransform, Button/TextMeshProUGUI
