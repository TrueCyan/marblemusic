...
description: Unity 프리팹(.prefab), 씬(.unity), ScriptableObject(.asset) 파일을 편집합니다. prefab-tool을 사용하여 프리팹 분석, JSON 변환, GameObject 생성/수정, UI 레이아웃 조정, 컴포넌트 추가, ScriptableObject 생성/수정 등의 작업을 수행합니다. 사용자가 Unity 프리팹, 씬, 또는 ScriptableObject 작업을 요청할 때 사용하세요.
---
...
ScriptableObject(.asset) 파일도 프리팹/씬과 동일한 Unity YAML 형식을 사용합니다. prefab-tool로 직접 편집하거나 YAML을 직접 작성할 수 있습니다.

### ScriptableObject YAML 구조
...
- `m_Name`: Inspector에 표시될 에셋 이름
- 나머지: 스크립트의 `[SerializeField]` 필드들

### ScriptableObject 직접 생성

#### 단순 ScriptableObject (BeatInstrumentData 예시)
...
MonoBehaviour:
  ...
  prefab: {fileID: 0}
  icon: {fileID: 0}
  soundClip: {fileID: 8300000, guid: AUDIO_CLIP_GUID, type: 3}
  volumeMultiplier: 1
```

#### 복잡한 ScriptableObject (MelodyInstrumentData 예시)
...
MonoBehaviour:
  ...
  m_Enabled: 1
  m_EditorHideFlags: 0
  m_Script: {fileID: 11500000, guid: c3d4e5f6789012345678abcdef012345, type: 3}
  m_Name: Piano
  m_EditorClassIdentifier:
  ...
  samples:
  - midiNote: 60
    clip: {fileID: 8300000, guid: AUDIO_CLIP_GUID_C4, type: 3}
    volumeAdjust: 1
  - midiNote: 64
    clip: {fileID: 8300000, guid: AUDIO_CLIP_GUID_E4, type: 3}
    volumeAdjust: 1
  volumeMultiplier: 1
  maxPitchShiftSemitones: 6
```
...
create_scriptable_object(
    "Assets/ScriptableObjects/Kick.asset",
    "b2c3d4e5f6789012345678abcdef0123",  # BeatInstrumentData GUID
    "Kick",
    {
        "instrumentName": "Kick",
        "prefab": "{fileID: 0}",
        "icon": "{fileID: 0}",
        "soundClip": "{fileID: 8300000, guid: abc123..., type: 3}",
        "volumeMultiplier": "1"
    }
)
```
...
이 프로젝트의 악기 관련 스크립트 GUID:

| 스크립트 | GUID | 설명 |
|----------|------|------|
| InstrumentData | `a1b2c3d4e5f6789012345678abcdef01` | 기본 클래스 (abstract) |
| BeatInstrumentData | `b2c3d4e5f6789012345678abcdef0123` | 비트 악기 SO |
| MelodyInstrumentData | `c3d4e5f6789012345678abcdef012345` | 멜로디 악기 SO |
